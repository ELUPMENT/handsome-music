<template>
    <el-container style="margin-top: 25px;marginn-bottom: 45px;">
      ...
      <el-main>
        ...
        <el-row style="border-bottom: 2px solid rgb(230,230,230)">
          <p style="margin-bottom: 10px; font-size: 22px;">
            推荐歌单
          </p>
        </el-row>
        ...
        <el-row :gutter="40" style="margin-top: 25px;">
          <el-col :span="8" v-for="item in privateList" :key="item.id">
            ...
            <h1 @click="toVideoPage(item.id)" style="cursor:pointer;overflow: hidden;
		text-overflow: ellipsis;white-space: nowrap;">{{item.name}}</h1>
          </el-col>
        </el-row>

        <!-- 最新音乐模块渲染 -->
        <el-rows style="border-bottom: 2px solid rgb(230,230,230)">
                    ...
                    <el-table-column>
                      <template scope="scope">
                        <el-image :src="scope.row.picUrl" @click="changeUrl(scope.row.id)"
                                  style="width: 75px;height: 75px;cursor:pointer;"></el-image>
                        <i class="el-icon-video-play" @click="changeUrl(scope.row.id)
                        "  style="position: absolute;top: 35px;font-size: 25px;left:35px;z-index:10;cursor:pointer">
                          
                        </i>
                        <span style="position: absolute;top: 25px;font-size: 15px;font-weight: 500">{{scope.row.name}}</span>
                        <span @click="toSingerPage(scope.row)"
                        style="cursor:pointer;position: absolute;bottom: 25px;font-size: 12px;font-weight: 500">{{scope.row.song.artists[0].name}}</span>
                      </template>
                    </el-table-column>
                  </el-table>
                </el-col>
