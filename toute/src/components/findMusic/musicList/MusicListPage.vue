<template>
  <el-container style="margin-bottom: 55px;">
      <el-header>
                <p>
          热门标签：
          <span id="cats" v-for="(item,index) in hotMusicListTags" 
          :class="'cat'+index" 
      ...
      <el-main>
          <el-row :gutter="10">
              <el-col :span="4" v-for="(item,index) in goodMusicList" :key="index" style="margin-top: 20px; position:relative;">
                  <!-- 播放量 -->
                  ...
                  <h1 @click="toSongListPage(item.id)" style="cursor:pointer;overflow: hidden;
